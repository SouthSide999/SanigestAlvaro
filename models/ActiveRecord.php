...
class ActiveRecord
{
    ...
    public static function paginarBusqueda($por_pagina, $offset, $filtro = [])
    {
        ...
        if (!empty($filtro['nombre'])) {
            $nombre = self::$db->escape_string($filtro['nombre']);
            $where .= " WHERE nombre LIKE '%$nombre%'";
        }

        $query = "SELECT * FROM " . static::$tabla . " $where ORDER BY id DESC LIMIT {$por_pagina} OFFSET {$offset}";
        $resultado = self::consultarSQL($query);
        return $resultado;
    }
    ...
    {
        $query = "SELECT * FROM " . static::$tabla . " WHERE LOWER(${columna}) LIKE LOWER('%${valor}%')";
        $resultado = self::consultarSQL($query);
        return $resultado;
    }
    ...
    {
        // Query que compara exactamente el valor sin importar mayúsculas/minúsculas
        $query = "SELECT * FROM " . static::$tabla . " WHERE LOWER(${columna}) = LOWER('${valor}')";
    ...
    public static function ultimoValor($columna, $tabla = null)
    {
        // Si no se pasa el nombre de la tabla, usar la tabla del modelo
        $tabla = $tabla ?? static::$tabla;

        // Crear la consulta para obtener el último valor de la columna
        $query = "SELECT MAX($columna) as $columna FROM " . $tabla;
        $resultado = self::consultarSQL($query);
